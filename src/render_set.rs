...
impl RenderSet{
    pub fn new(window: &Window, device: &wgpu::Device, config: &wgpu::SurfaceConfiguration) -> Self {
    ...
    pub fn render(&mut self, encoder: &mut CommandEncoder, particles_buffer: &wgpu::Buffer, surface: &wgpu::Surface) -> SurfaceTexture {

        let frame = surface.get_current_texture().expect("Timeout when acquiring next surface texture");
        ...
        encoder.push_debug_group("render particles");
        {
            ...
            render_pass.set_index_buffer(self.index_buffer.get_buffer().slice(..), wgpu::IndexFormat::Uint16);
